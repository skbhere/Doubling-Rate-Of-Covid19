...
@app.route("/drec")
def st():
    data = pd.read_csv("https://covid19-doublingrate.herokuapp.com/mdata")
    #data = Med()
    data = data[data.City != 'Unknown']
    data = data.drop(["Unnamed: 0"], axis=1)
    data = data.sort_values(by='Value', ascending=True)
...
def state():
    if request.method == 'POST':
        dis = request.form['dist']
        return redirect(url_for('table', messages=dis))
    else:
        ...
        State=sorted(State)
        messages= State[0]
        return render_template('stateindex.html', dist=State)
def post(self):
    dis = self.request.get('d')
    return redirect(url_for('table', messages=dis))
...
def table1():
    ...
    titles = ['Medical Efficiency', 'Doubling rate foe the last 7 days '])
